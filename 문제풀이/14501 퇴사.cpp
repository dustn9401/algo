...
#include <algorithm>
using namespace std;
int n, note[15], t[15], p[15];
int memo(int day) {
	if (day >= n) return 0;
	if (note[day] != -1) return note[day];
	note[day] = 0;
	if(day + t[day] <= n) note[day] = max(note[day], p[day] + memo(day + t[day]));
	if(day + 1 <= n) note[day] = max(note[day], memo(day + 1));
	return note[day];
}
int main()
{
